    ...
    const req = requestByProtocol(url, { method:'GET', headers }, (res)=>{
      ...
      let body=''; res.on('data',(c)=> body+=c); res.on('end', async ()=>{
        if([301,302,303,307,308].includes(status) && location && maxRedirects>0){
          ...
          try { const next = await followGet(nextUrl, headers, maxRedirects-1); return resolve(next); } catch(e){ return reject(e); }
        }
...
module.exports = async (req,res)=>{
  ...
  const API_KEY = process.env.OPENELECTRICITY_API_KEY; if(!API_KEY) return res.status(500).json({ ok:false, error:'OPENELECTRICITY_API_KEY not set' });
  try{
    const url = new URL(`${OE_BASE}/facilities`);
    const headers = { 'Authorization':`Bearer ${API_KEY}`, 'Accept':'application/json', 'User-Agent':'aed-ping/1.2' };
    const up = await followGet(url, headers);
    if(up.status===200){ return res.status(200).json({ ok:true, status: up.status, requestId: up.requestId, sample: up.body? up.body.slice(0,200): '' }); }
    return res.status(up.status||502).json({ ok:false, status: up.status, requestId: up.requestId, bodySnippet: up.body? up.body.slice(0,400): '', finalUrl: up.url });
  }catch(e){ return res.status(502).json({ ok:false, error:'network', message: e.message }); }
};
