...
async function fetchOpenElectricityData(startISO,endISO,apiKey,mode,interval){
  ...
  async function doRequest(grouped){
    const params=new URLSearchParams({metrics:'price',interval:interval||'1d',date_start:startISO,date_end:endISO});
...
function processResponse(apiResponse){
  const { json } = apiResponse || {}; if(!json || json.success===false) return {};
  ...
  json.data.forEach(series=>{
    ...
    series.results.forEach(r=>{
      const region = r?.columns?.network_region || r?.name || r?.id; if(!region || !REGIONS.includes(region)) return;
      ...
      points.forEach(pt=>{
        ...
        const d=new Date(ts); const key=`${d.getUTCFullYear()}-${String(d.getUTCMonth()+1).padStart(2,'0')}`;
        if(!buckets[region][key]) buckets[region][key]={year:d.getUTCFullYear(),month:d.getUTCMonth()+1,date:new Date(Date.UTC(d.getUTCFullYear(),d.getUTCMonth(),1)).toISOString(),prices:[],negativeCount:0,highCount:0,extremeCount:0,highPrices:[],extremePrices:[]};
        const m=buckets[region][key]; m.prices.push(val);
        if(val<0) m.negativeCount++; else if(val>=300 && val<1000){ m.highCount++; m.highPrices.push(val);} else if(val>=1000){ m.extremeCount++; m.extremePrices.push(val);} 
      });
    });
  });
  const out={};
  REGIONS.forEach(region=>{
    const months = Object.values(buckets[region]).map(m=>{
      const avg=m.prices.reduce((a,b)=>a+b,0)/m.prices.length; const max=Math.max(...m.prices); const n=m.prices.length;
      ...
      return { year:m.year, month:m.month, date:m.date, averagePrice:Number(avg.toFixed(2)), maxPrice:Number(max.toFixed(2)), priceEvents:{ negative:{count:m.negativeCount,percentage:((m.negativeCount/n)*100).toFixed(2)}, high:{count:m.highCount,percentage:((m.highCount/n)*100).toFixed(2),avgPrice:Number(avgHigh.toFixed(2))}, extreme:{count:m.extremeCount,percentage:((m.extremeCount/n)*100).toFixed(2),avgPrice:Number(avgExtreme.toFixed(2))} } };
...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    const mode=(req.query.group||'auto').toLowerCase(); const interval=req.query.interval;
    let startISO=req.query.date_start, endISO=req.query.date_end;
    if(!startISO||!endISO){ const end=new Date(); end.setUTCDate(end.getUTCDate()-2); const start=new Date(end); start.setUTCFullYear(end.getUTCFullYear()-years); startISO=isoMidnightUTC(start); endISO=isoMidnightUTC(end); }
    ...
    if(!totalMonths) return res.status(404).json({ error:'No data after processing', startISO, endISO });
    return res.status(200).json({ data: processed, fetchedAt: new Date().toISOString(), source:'OpenElectricity v4', dateRange:{ start:startISO, end:endISO } });
  }catch(e){
    ...
    console.error('[FATAL]', e); return res.status(500).json({ error:'Unhandled server error', message:e?.message||String(e) });
  }
};
